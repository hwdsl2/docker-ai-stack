...
本指南協助你在變更設定或提交 issue 之前診斷 Docker AI Stack 問題。
...
公網暴露可選服務時，請為預設允許本機無驗證使用的服務設定 API 金鑰或代理驗證。
...
排查問題時，除非已有目前備份，否則不要刪除磁碟區。刪除磁碟區可能會移除 API 金鑰、模型快取、AnythingLLM 資料、LiteLLM 設定、MCP Gateway 設定和 Caddy 憑證。
