...
Это руководство помогает диагностировать проблемы Docker AI Stack перед изменением конфигурации или созданием issue.
...
При публикации опциональных сервисов в интернет задайте API-ключи или proxy-аутентификацию для сервисов, которые по умолчанию допускают локальное использование без аутентификации.
...
Не удаляйте тома при диагностике, если у вас нет актуальной резервной копии. Удаление томов может удалить API-ключи, кэши моделей, данные AnythingLLM, конфигурацию LiteLLM, настройки MCP Gateway и сертификаты Caddy.
...
Создавайте issue в `docker-ai-stack` для:
