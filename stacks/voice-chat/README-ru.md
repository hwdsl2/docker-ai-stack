...
Откройте `http://<server-ip>:3001` в браузере. Провайдер LLM, базовый URL и модель уже настроены.
...
```bash
# Из живых логов (показывается только при первом запуске):
docker compose logs anythingllm | grep -A2 "FIRST RUN"

# Или в любой момент из тома данных:
...
Для развёртываний с выходом в интернет используйте включённый Caddy overlay для автоматического HTTPS. Выполняйте эти команды из каталога `stacks/voice-chat`. В режиме прокси Caddy является единственным публичным слушателем на портах `80` и `443`; прямые порты AnythingLLM и LiteLLM заново привязываются к `127.0.0.1`. По умолчанию прокси открывает только AnythingLLM; Whisper и Kokoro остаются привязанными согласно compose-файлу этого подстека.
