...
Откройте `http://<IP-сервера>:3001` в браузере. Провайдер LLM, базовый URL и модель уже предварительно настроены.
...
> **Совет:** При предоставлении доступа к AnythingLLM за пределами `localhost` или доверенной локальной сети размещайте его за обратным прокси с TLS, чтобы пароль шифровался при передаче. Для развёртываний с выходом в интернет также измените `"3001:3001/tcp"` и `"4000:4000/tcp"` на `"127.0.0.1:3001:3001/tcp"` и `"127.0.0.1:4000:4000/tcp"` в `docker-compose.yml`, чтобы предотвратить прямой доступ к незашифрованным портам. См. ниже [Использование обратного прокси](#использование-обратного-прокси).
