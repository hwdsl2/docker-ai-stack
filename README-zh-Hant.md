...
- 零配置：所有服務在首次啟動時自動配置
- 安全：AnythingLLM 預設啟用密碼保護，Ollama、LiteLLM 和 MCP Gateway 自動產生 API 金鑰
...
在瀏覽器中開啟 `http://<server-ip>:3001`。AnythingLLM 已預設透過 LiteLLM 連接本機大型語言模型。首次啟動時，可能需要幾分鐘才能就緒（使用 `docker logs anythingllm` 檢視進度）。
